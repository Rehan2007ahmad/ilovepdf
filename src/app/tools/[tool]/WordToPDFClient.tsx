  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      if (
        ...
        setError("Only Word files (.docx, .doc) are allowed!");
        setFile(null);
        setOutputName("");
        return;
      }
  ...
  return (
    <section className="relative bg-gray-900 text-yellow-400 overflow-hidden min-h-screen flex flex-col items-center justify-center px-4">
      <div className="relative z-10 max-w-3xl w-full text-center">
        <h1 className="text-5xl md:text-6xl font-bold mb-6">Word to Pdf</h1>
        <p className="text-lg md:text-xl mb-8 text-gray-300">
          Convert your Word documents to PDF instantly. Upload your Word file
          and download the PDF in seconds!
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            <FaUpload /> {file ? "Change File" : "Upload Word"}
            <input
              type="file"
              accept=".doc,.docx"
              className="hidden"
