...
export default function JPGToWEBP({ tool }: { tool?: string }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/tools/jpg-to-webp", {
        method: "POST",
        body: formData,
      });
      ...
      link.href = url;
      link.download = (outputName || "converted") + ".webp";
  ...
  return (
    <section className="relative bg-gray-900 text-yellow-400 overflow-hidden min-h-screen flex flex-col items-center justify-center px-4">
      <div className="relative z-10 max-w-3xl w-full text-center">
        <h1 className="text-5xl md:text-6xl font-bold mb-6">JPG to WEBP</h1>
        <p className="text-lg md:text-xl mb-8 text-gray-300">
          Convert your JPG images to WEBP format instantly. Upload your image and get the result in seconds!
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            <FaUpload /> {file ? "Change File" : "Upload JPG"}
            <input
              type="file"
              accept=".jpg"
              className="hidden"
