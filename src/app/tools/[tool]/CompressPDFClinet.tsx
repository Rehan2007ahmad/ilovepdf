...
export default function CompressPDF() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = (outputName || "compressed") + ".pdf";
  ...
  return (
    <section className="relative bg-gray-900 text-yellow-400 overflow-hidden min-h-screen flex flex-col items-center justify-center px-4">
      <div className="relative z-10 max-w-3xl w-full text-center">
        <h1 className="text-5xl md:text-6xl font-bold mb-6">Compress PDF</h1>
        <p className="text-lg md:text-xl mb-8 text-gray-300">
          Reduce the size of your PDF files instantly while keeping the quality.
          Upload your PDF and download a smaller version in seconds!
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            <FaUpload /> {file ? "Change File" : "Upload PDF"}
            <input
              type="file"
              accept=".pdf"
              className="hidden"
